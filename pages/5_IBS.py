...
from ibs_engine import run_ibs_pathway

st.set_page_config(page_title="IBS Pathway", layout="wide")
st.title("Irritable Bowel Syndrome (IBS) Pathway Assessment")
...
outputs, logs, overrides = run_ibs_pathway(st.session_state.patient_data)
...
def render_input_field(field_name):
    ...
    if any(keyword in field_name for keyword in ["percent", "per_week", "per_day", "duration", "days", "months"]):
