...
with left:
    ...
    if ppi_od_adequate is True or ppi_bid_adequate is True:
        ...
        if ppi_od_adequate is False and ppi_bid_adequate is False:
...
with right:
    if not st.session_state.dys_has_run:
        ...
        v9_to_10 = v9
        v10 = ppi_od_ok or ppi_bid_ok or v9_to_10
        ...
        def vline(x, y1, y2, vis, urgent=False, exit_=False, label="", label_side="right"):
            ...
            svg.append(f'<line x1="{x}" y1="{y1}" x2="{x}" y2="{y2}" '
                        f'stroke="{s}" stroke-width="2" {dash_style} marker-end="url(#{m})"/>')
            if label:
        ...
        def elbow(x1, y1, x2, y2, vis, urgent=False, exit_=False, label=""):
            ...
            svg.append(f'<polyline points="{x1},{y1} {x2},{y1} {x2},{y2}" '
                        f'fill="none" stroke="{s}" stroke-width="2" {dash_style} marker-end="url(#{m})"/>')
            if label:
        ...
        d7_mid_y = Y["d7"] + DH//2
        n10_left_x = CX - NW//2
        n10_mid_y = Y["n10"] + NH//2
        bypass_x = LX + EW//2

        s_od = C_MAIN if ppi_od_ok else "#64748b"
        m_od = "mg" if ppi_od_ok else "ma"
        dash_od = "" if ppi_od_ok else 'stroke-dasharray="5,3"'

        svg.append(
            f'<polyline points="{d7_left_x},{d7_mid_y} {bypass_x},{d7_mid_y} '
            f'{bypass_x},{n10_mid_y} {n10_left_x},{n10_mid_y}" '
            f'fill="none" stroke="{s_od}" stroke-width="2" '
            f'{dash_od} marker-end="url(#{m_od})"/>'
        )
        svgt((d7_left_x + bypass_x)//2, d7_mid_y - 5, "Yes", s_od, 10, True)
        ...
        s_bid = C_MAIN if ppi_bid_ok else "#64748b"
        m_bid = "mg" if ppi_bid_ok else "ma"
        dash_bid = "" if ppi_bid_ok else 'stroke-dasharray="5,3"'

        svg.append(
            f'<polyline points="{d8_left_x},{d8_mid_y} {bypass_x},{d8_mid_y} '
            f'{bypass_x},{n10_mid_y} {n10_left_x},{n10_mid_y}" '
            f'fill="none" stroke="{s_bid}" stroke-width="2" '
            f'{dash_bid} marker-end="url(#{m_bid})"/>'
        )
        svgt((d8_left_x + bypass_x)//2, d8_mid_y - 5, "Yes", s_bid, 10, True)
        ...
        vline(CX, Y["n9"]+NH, Y["n10"], v9_to_10)

        # ── NODE 10: Titrate Down / PPI Maintenance ──────────────────────────
        rect_node(CX-NW//2, Y["n10"], NW, NH, nc(v10), "Titrate Down /", "PPI Maintenance")
        ...
        STATUS_CODES = {
            "DYSPEPSIA_ENTRY_MET",
            "NOT_PREDOMINANT_GERD",
            "NO_ALARM_FEATURES",
            "HPYLORI_NEGATIVE_OR_TREATED",
            "MEDICATION_LIFESTYLE_REVIEWED",
        }
