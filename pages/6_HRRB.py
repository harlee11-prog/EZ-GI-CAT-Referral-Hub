...
from hrrb_engine import run_hrrb_pathway

st.set_page_config(page_title="HRRB Pathway", layout="wide")
st.title("High Risk Rectal Bleeding (HRRB) Pathway Assessment")
...
outputs, logs, overrides = run_hrrb_pathway(st.session_state.patient_data)
...
def render_input_field(field_name):
    ...
    if field_name == "sex":
        val = st.selectbox(display_name, options=[None, "male", "female"], index=0 if current_value is None else (1 if current_value == "male" else 2), key=field_name)
        if val is not None:
            st.session_state.patient_data[field_name] = val
    elif any(keyword in field_name for keyword in ["percent", "per_week", "per_day", "duration", "days", "months", "ago"]):
