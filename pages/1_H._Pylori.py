...
def build_h_pylori_markdown(patient_data, outputs, overrides, notes: str) -> str:
    ...
    lines.append(
        f"- **H. pylori test result:** "
        f"{_safe_text(patient_data.get('hp_test_result')) or 'Not tested'}"
    )
    lines.append(
        f"- **Test type:** {_safe_text(patient_data.get('hp_test_type')) or 'N/A'}"
    )
    lines.append(
        f"- **Treatment line:** "
        f"{_safe_text(patient_data.get('treatment_line')) or 'N/A'}"
    )
    lines.append(
        f"- **Penicillin allergy:** "
        f"{_safe_text(patient_data.get('penicillin_allergy')) or 'No / not documented'}"
    )
    lines.append(
        f"- **Pregnant:** "
        f"{_safe_text(patient_data.get('pregnant')) or 'No / not documented'}"
    )
    lines.append(
        f"- **Breastfeeding:** "
        f"{_safe_text(patient_data.get('breastfeeding')) or 'No / not documented'}"
    )
    lines.append(
        f"- **Symptoms persist:** "
        f"{_safe_text(patient_data.get('symptoms_persist')) or 'Unknown'}"
    )
    lines.append(
        f"- **Eradication test result:** "
        f"{_safe_text(patient_data.get('eradication_test_result')) or 'Not done'}"
    )
    lines.append("")
            ...
            if isinstance(o, Action):
                ...
                if isinstance(o.details, dict):
                    ...
                    if regimen_key and regimen_key in REGIMEN_DETAILS:
                        ...
                        lines.append(
                            f"  - Regimen: **{_safe_text(r.get('name'))}**"
                        )
                        lines.append(
                            f"  - Duration: {_safe_text(r.get('duration'))}"
                        )
                        lines.append(
                            f"  - Approx cost: {_safe_text(r.get('approx_cost'))}"
                        )
                        ...
                        if r.get("notes"):
                            lines.append(
                                f"  - Regimen note: {_safe_text(r.get('notes'))}"
                            )
                    ...
                    for k, v in o.details.items():
                        ...
                        if isinstance(v, list):
                            for item in v:
                                lines.append(
                                    f"  - {_safe_text(k).replace('_', ' ').title()}: "
                                    f"{_safe_text(item)}"
                                )
                ...
                if getattr(o, "actions", None):
                    for a in o.actions:
                        lines.append(
                            f"  - Follow-up: {_safe_text(a.label)}"
                        )
                ...
                if getattr(o, "suggested_actions", None):
                    for a in o.suggested_actions:
                        lines.append(
                            f"  - Suggested action: {_safe_text(a.label)}"
                        )
    lines.append("")
    ...
    lines.append(
        ...
        else "No clinician notes entered."
    )
    lines.append("")
    return "\n".join(lines)
...
# ── HANDOFF INBOUND ──────────────────────────────────────────────────────────
_hp_handoff = apply_handoff("1_H._Pylori")
if _hp_handoff:
    transferred = []

    if _hp_handoff.get("age") is not None:
        st.session_state["_ho_age"] = int(_hp_handoff["age"])
        transferred.append("age")
    if _hp_handoff.get("sex"):
        st.session_state["_ho_sex"] = _hp_handoff["sex"]
        transferred.append("sex")

    hp_flag = _hp_handoff.get("h_pylori_result_positive")
    if hp_flag is not None:
        st.session_state["_ho_hpresult"] = "Positive" if hp_flag else "Negative"
        transferred.append("h_pylori_result_positive")
    elif _hp_handoff.get("hp_test_result") in ("positive", "negative"):
        st.session_state["_ho_hpresult"] = _hp_handoff["hp_test_result"].capitalize()
        transferred.append("hp_test_result")

    alarm_map = [
        ("actively_bleeding_now", "ho_al_bleeding"),
        ("family_history_upper_gi_cancer_first_degree", "ho_al_cancer"),
        ("family_history_esophageal_or_gastric_cancer_first_degree", "ho_al_cancer"),
        ("age_over_60_new_persistent_symptoms_over_3_months", "ho_al_age60"),
        ("symptom_onset_after_age_60", "ho_al_age60"),
        ("unintended_weight_loss", "ho_al_wl"),
        ("dysphagia", "ho_al_dys"),
        ("progressive_dysphagia", "ho_al_dys"),
        ("persistent_vomiting", "ho_al_vomit"),
        ("black_stool_or_blood_in_vomit", "ho_al_bleed"),
        ("iron_deficiency_anemia_present", "ho_al_ida"),
    ]
    for src, dst in alarm_map:
        if _hp_handoff.get(src):
            st.session_state[f"_{dst}"] = True
            transferred.append(src)

    show_handoff_banner("GERD / Dyspepsia", transferred)
...
with left:
    ...
    age = st.number_input(
        ...
        1,
        120,
        value=st.session_state.pop("_ho_age", 52),
    )
    sex = st.selectbox(
        "Sex",
        ["male", "female"],
        index=["male", "female"].index(
            st.session_state.pop("_ho_sex", "male")
        ),
    )
    ...
    hp_result_sel = st.selectbox(
        ...
        index=["Not tested", "Positive", "Negative"].index(
            st.session_state.pop("_ho_hpresult", "Not tested")
        ),
    )
    hp_result_map = {
        "Not tested": None,
    ...
    al_family_cancer = st.checkbox(
        "Family hx esophageal/gastric cancer",
        value=st.session_state.pop("_ho_al_cancer", False),
    )
    al_ulcer_hx = st.checkbox("Personal history of peptic ulcer disease")
    al_age_symptoms = st.checkbox(
        "Age ≥60 with new persistent symptoms",
        value=st.session_state.pop("_ho_al_age60", False),
    )
    al_weight_loss = st.checkbox(
        "Unintended weight loss >5%",
        value=st.session_state.pop("_ho_al_wl", False),
    )
    al_dysphagia = st.checkbox(
        "Progressive dysphagia",
        value=st.session_state.pop("_ho_al_dys", False),
    )
    al_vomiting = st.checkbox(
        "Persistent vomiting (not cannabis-related)",
        value=st.session_state.pop("_ho_al_vomit", False),
    )
    al_gi_bleed = st.checkbox(
        "Black stool or blood in vomit",
        value=st.session_state.pop("_ho_al_bleed", False),
    )
    al_ida = st.checkbox(
        "Iron deficiency anemia",
        value=st.session_state.pop("_ho_al_ida", False),
    )
    al_concern = st.checkbox("Clinician concern for serious pathology")
    ...
    st.markdown("**Eradication Follow-up**")
    ...
    symptoms_persist_sel = st.selectbox(
...
# ═════════════════════════════════════════════════════════════════════════════
with right:
    if not st.session_state.hp_has_run:
        ...
        patient_data = {
            ...
            "symptoms_persist": sp_map.get(symptoms_persist_sel),
        ...
        # ── Outbound: H. pylori → Dyspepsia ─────────────────────────────────
        hp_neg_to_dys = any(
            isinstance(o, Action) and o.code == "ROUTE_DYSPEPSIA_PATHWAY"
            for o in outputs
        ) or (
            patient_data.get("hp_test_result") == "negative"
            and patient_data.get("dyspepsia_symptoms")
        )
        if hp_neg_to_dys:
            st.info(
                "H. pylori is negative with dyspepsia symptoms – you can continue "
