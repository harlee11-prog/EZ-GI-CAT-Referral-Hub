...
with col4:
    with st.container(border=True):
        ...
        with pdf4:
            st.link_button("📄 PDF", "https://www.albertahealthservices.ca/assets/info/hp/cancer/if-hp-cancer-cr-chronic-diarrhea-pathway.pdf", use_container_width=True)
...
with col5:
    with st.container(border=True):
        ...
        with pdf5:
            st.link_button("📄 PDF", "https://www.albertahealthservices.ca/assets/info/hp/cancer/if-hp-cancer-cr-ibs-pathway.pdf", use_container_width=True)

with col6:
    with st.container(border=True):
        st.markdown("### 🩺 High Risk Rectal Bleeding")
        nav6, pdf6 = st.columns([2, 1])
        with nav6:
            st.page_link("pages/6_HRRB.py", label="Open →", use_container_width=True)
        with pdf6:
            st.link_button("📄 PDF", "https://www.albertahealthservices.ca/assets/info/hp/cancer/if-hp-cancer-cr-hrrb-pathway.pdf", use_container_width=True)
